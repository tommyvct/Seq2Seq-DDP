...
def pack_batch(batch, max_source_length, max_target_length, eos_id):
    packed = {"input_ids": [], "attention_mask": [], "labels": []}
    buf_inp, buf_lbl = [], []
    
    for inp, lbl in zip(batch["input_ids"], batch["labels"]):
        inp = inp + [eos_id]
        # lbl already ends with EOS
        if buf_inp and (
            len(buf_inp) + len(inp) > max_source_length
            or len(buf_lbl) + len(lbl) > max_target_length
        ):
            packed["input_ids"].append(buf_inp[:])
            packed["attention_mask"].append([1] * len(buf_inp))
            packed["labels"].append(buf_lbl[:])
            buf_inp.clear()
            buf_lbl.clear()
...
def pack_dataset(tokenized_dataset, max_source_length, max_target_length, eos_id, num_proc=1):
    print(f"  Packing across {num_proc} processes using HF map...")
...
def main(args):
    ...
    tokenized_tmp_path = os.path.join(processed_data_dir, "_tokenized_tmp")
    ...
    if os.path.exists(tokenized_tmp_path):
        print(f"Tokenized data already exists at {tokenized_tmp_path}. Loading from disk...")
        combined_train = datasets.load_from_disk(tokenized_tmp_path)
        combined_val = None # We'll skip val tokenization later if this is the path taken, or handle it differently.
    else:
        ...
        # Save tokenized data to disk and reload as memory-mapped Arrow, so that
        # the packing loop doesn't hold both the full tokenized dataset AND the
        # packed output in RAM simultaneously (that caused OOM at 720G).
        print("Saving tokenized data to disk to free RAM before packing...")
        combined_train.save_to_disk(tokenized_tmp_path)
        del combined_train
        combined_train = datasets.load_from_disk(tokenized_tmp_path)
        print(f"Reloaded {len(combined_train):,} tokenized examples (memory-mapped)")
    ...
    # Clean up temporary tokenized data
    import shutil
    shutil.rmtree(tokenized_tmp_path, ignore_errors=True)
    print("Cleaned up temporary tokenized data")
